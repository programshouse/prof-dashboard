...
export default function SubscriberForm({ subscriberId, initialValues, onSuccess }) {
  ...
  return (
    <PageLayout title={subscriberId ? "Edit Subscriber | ProfMSE" : "Add Subscriber | ProfMSE"}>
      ...
      {loading ? (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 text-center">Loading…</div>
      ) : (
        <AdminForm title="Subscriber" onSubmit={submit} onCancel={cancel} submitText={saving ? "Saving…" : subscriberId ? "Update" : "Create"}>
          {!!error && (
            <div className="mb-4 rounded-lg border border-red-300 bg-red-50 px-3 py-2 text-sm text-red-700 dark:border-red-700/40 dark:bg-red-900/20 dark:text-red-300">{error}</div>
          )}
          <div>
